<script>    import WeatherCard from './lib/WeatherCard.svelte';
...
<main><h1>Weather Forecast</h1>
    <div class="search"><input bind:value={city} placeholder="Enter city"/>
...
.search {
    ...
    margin-bottom: 30px;
    gap: 0; /* Убираем промежуток между input и кнопкой */
}
...
.search input {
    ...
    border-right: none; /* Убираем правую границу, чтобы соединить с кнопкой */
    border-radius: 30px 0 0 30px; /* Закругляем только левую сторону */
    outline: none;
    box-shadow: none;
...
.search button {
    ...
    font-size: 16px;
    border: none;
    border-radius: 0 30px 30px 0; /* Закругляем только правую сторону */
    cursor: pointer;
